 ...
 * queries easier. Added bulk insert and apply batch operations.
 */
public abstract class AsyncQueryHandler extends Handler {
    ...
    private static final int EVENT_ARG_APPLY_BATCH = 6; // TODO Test batch operation
        ...
        public void handleMessage(Message msg) {
            ...
            switch (event) {
                ...
                case EVENT_ARG_UPDATE:
                    args.result = resolver.update(args.uri, args.values[0], args.selection,
                            args.selectionArgs);
                    break;
                ...
                case EVENT_ARG_APPLY_BATCH:
                    ...
                    try {
                         contentProviderResults = resolver.applyBatch(args.authority, args.operations);
                    } catch (Exception e) {
                        Log.w(TAG, "Exception thrown during handling EVENT_ARG_APPLY_BATCH", e);
     ...
     */
    public final void startBulkInsert(int token, Object cookie, Uri uri, ContentValues[] values) {
        // Use the token as what so cancelOperations works properly
        Message msg = mWorkerThreadHandler.obtainMessage(token);
    ...
    /**
     * This method begins an asynchronous batch operation. When the batch operation is
     * done {@link #onApplyBatch} is called.
     *
     * @param token A token passed into {@link #onApplyBatch} to identify batch operation.
     * @param cookie An object that gets passed into {@link #onBulkInsertComplete}
     * @param authority the authority of the ContentProvider to which this batch should be applied
     * @param operations the operations to apply
     */
    public final void startApplyBatch(int token, Object cookie, String authority, ArrayList<ContentProviderOperation> operations) {
        // Use the token as what so cancelOperations works properly
        Message msg = mWorkerThreadHandler.obtainMessage(token);
    ...
    /**
     * Called when an asynchronous batch operation is completed.
     *
     * @param token the token to identify the query, passed in from
     *        {@link #startApplyBatch}.
     * @param cookie the cookie object that's passed in from
     *        {@link #startApplyBatch}.
     * @param result the result returned from the batch operation
     */
    protected void onApplyBatch(int token, Object cookie, ContentProviderResult[] result) {
        // Empty
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (event) {
            ...
            case EVENT_ARG_APPLY_BATCH:
                onApplyBatch(token, args.cookie, (ContentProviderResult[]) args.result);
                break;
        }
    }
